...
// Define the system message to guide the AI's behavior as a study bot
const systemMessage = {
  role: "system",
  content: `
You are an expert study assistant and tutor. Your goal is to help the user learn and understand concepts clearly and patiently.
...
export async function POST(req: Request) {
  try {
    ...
    // Call the LLM with the full messages including system prompt
    const stream = await llm.stream(messagesWithSystem);
      ...
      async start(controller) {
        try {
          for await (const chunk of stream) {
            // chunk.content can be string or array of strings
